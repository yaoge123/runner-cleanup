...
CONFIG_ONLY_LOG_DIR="${TMP_DIR}/config-only-logs"
CONFIG_ONLY_LOG_FILE="${CONFIG_ONLY_LOG_DIR}/runner-cleanup.log"
CONFIG_ONLY_CONFIG_FILE="${TMP_DIR}/config-only.conf"

cat > "${CONFIG_ONLY_CONFIG_FILE}" <<EOF
ENABLE_IMAGE_CLEANUP=0
ENABLE_DOCKER_CACHE_CLEANUP=0
ENABLE_LOCAL_CACHE_CLEANUP=0
RUNNER_CLEANUP_LOG_DIR=${CONFIG_ONLY_LOG_DIR}
RUNNER_CLEANUP_LOG_FILE=${CONFIG_ONLY_LOG_FILE}
EOF

RUNNER_CLEANUP_CONFIG="${CONFIG_ONLY_CONFIG_FILE}" \
  bash "${REPO_DIR}/run.sh"

assert_file_contains "${CONFIG_ONLY_LOG_FILE}" "config=${CONFIG_ONLY_CONFIG_FILE}"
